...
class Boid(object):
    ...
    def fly_middle(self, Flock, fly_middle_strength=0.01):
        ...
        for boid in Flock.boids:
            self.x_velo = self.x_velo + (boid.x_coord - self.x_coord) * fly_middle_reg
            self.y_velo = self.y_velo + (boid.y_coord - self.y_coord) * fly_middle_reg
    ...
    def fly_away(self, Flock, distance_limit=100, scaling=2):
        for boid in Flock.boids:
            ...
            if distance < distance_limit:
                self.x_velo = self.x_velo + (self.x_coord - boid.x_coord)
                self.y_velo = self.y_velo + (self.y_coord - boid.y_coord)
    ...
    def match_speed(self, Flock, speed_match_strength=0.125, scaling=2, distance_limit=10000):
        ...
        for boid in Flock.boids:
            distance = (boid.x_coord - self.x_coord) ** scaling + (boid.y_coord - self.y_coord) ** scaling
            if distance < distance_limit:
                self.x_velo = self.x_velo + (boid.x_velo - self.x_velo) * speed_match_reg
                self.y_velo = self.y_velo + (boid.y_velo - self.y_velo) * speed_match_reg


    def move(self):
        self.x_coord = self.x_coord + self.x_velo
        self.y_coord = self.y_coord + self.y_velo
